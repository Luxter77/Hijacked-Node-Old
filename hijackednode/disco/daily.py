...
async def long_sleep(arg):
    hours = 60 * 60
    async def bg():
        while True: await asyncio.sleep(12 * hours)
...
async def Wednesday(config: CONF0, bot: discord.ext.commands.Bot):
    while True:
        day = config.DailyDict[dt.datetime.today.weekday()]
        if day.enabled:
            for chan in config.DailyChan:
                if(type(chan) is discord.TextChannel):
                    async with chan.typing():
                        await chan.send(content=day.msg, file=discord.File(day.filename))
                elif((type(chan) is str) or (type(chan) is int)):
                    async with bot.get_channel(chan).typing():
                        await bot.get_channel(chan).send(content=day.msg, file=discord.File(day.filename))
                else:
